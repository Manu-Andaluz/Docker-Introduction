...
filmRouter.get("/", async (req, res) => {
  const promise = await axios.get("http://localhost:5005/Film");
  res.status(200).send(promise.data);
});

filmRouter.get("/:id", async (req, res) => {
  const id = req.params.id;
  const promise = await axios.get(`http://localhost:5005/Film/${id}`);
  res.status(200).send(promise.data);
});

filmRouter.post("/", async (req, res) => {
  const data = req.body;
  const promise = await axios.post("http://localhost:5005/Film", data);
  res.status(200).send(promise.data);
});
